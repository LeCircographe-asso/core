...
RSpec.describe AttendanceManagement::AttendanceRemover do
  ...
  describe '#call' do
    context 'with pack10 book of entry' do
      ...
      let!(:book_of_entry) { create(:book_of_entry, person: person, subscription_plan: pack_plan, sessions_remaining: 4) }
      let!(:attendance) do
        create(:attendance, person: person, attendance_list: attendance_list, book_of_entry: book_of_entry, date: Date.current)
      end

      it 'removes attendance and refunds a session' do
        initial_after_creation = book_of_entry.reload.sessions_remaining
        ...
        expect(book_of_entry.reload.sessions_remaining).to eq(initial_after_creation + 1)
    ...
    context 'with day pass' do
      ...
      let!(:book_of_entry) do
        create(:book_of_entry, person: person, subscription_plan: day_plan, sessions_remaining: 0, status: :consumed, expires_at: Date.current.end_of_day)
      end
      let!(:attendance) { create(:attendance, person: person, attendance_list: attendance_list, book_of_entry: book_of_entry) }

      it 'reactivates the day pass on refund' do
        described_class.new(attendance_id: attendance.id).call

        expect(book_of_entry.reload.sessions_remaining).to eq(1)
        expect(book_of_entry.status).to eq('active')
      end
    end
    ...
    context 'with annual subscription' do
      ...
      let!(:book_of_entry) do
        create(:book_of_entry, person: person, subscription_plan: annual_plan, sessions_remaining: nil, expires_at: 1.year.from_now)
      end
      let!(:attendance) { create(:attendance, person: person, attendance_list: attendance_list, book_of_entry: book_of_entry) }
      ...
      it 'removes attendance without altering unlimited plan' do
        ...
        expect(book_of_entry.reload.sessions_remaining).to be_nil
        expect(book_of_entry.status).to eq('active')
      end
    end
