namespace :assets do
  ...
  task :doctor do
    ...
    if !Dir.exist?(fonts_dir) || Dir.children(fonts_dir).grep(/\.(woff2|woff|otf)$/i).empty?
      errors << 'No fonts found in app/assets/fonts (expected .woff2/.woff/.otf)'
    end

    layout = Rails.root.join('app/views/layouts/application.html.erb')
    layout_text = File.read(layout)
    unless layout_text.include?('stylesheet_link_tag "tailwind"')
      errors << 'Layout: tailwind stylesheet_link_tag missing'
    end
    unless layout_text.include?('stylesheet_link_tag "flowbite"')
      errors << 'Layout: flowbite stylesheet_link_tag missing'
    end
    unless layout_text.include?('javascript_include_tag "flowbite.turbo.min"')
      errors << 'Layout: flowbite.turbo.min javascript_include_tag missing'
    end
